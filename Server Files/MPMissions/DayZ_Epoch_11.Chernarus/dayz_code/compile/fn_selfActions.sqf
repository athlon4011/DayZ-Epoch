...
// ---------------------------------------Krixes Self Bloodbag Start------------------------------------
    ...
    if((speed player <= 1) && hasBagItem && _canDo) then {
        if (s_player_selfBloodbag < 0) then {
            s_player_selfBloodbag = player addaction[("<t color=""#c70000"">" + ("Self Bloodbag") +"</t>"),"Plugins\bloodbag\player_selfbloodbag.sqf","",5,false,true,"", ""];
        };
    } else {
	...
	if (_isMan and !_isPZombie and _traderType in serverTraders) then {
		...
		if (s_player_parts_crtl < 0) then {
			...
			if(_humanity_logic) then {
				...
				// Database menu 
				_buy = player addAction ["Trader Menu", "Plugins\traders\show_dialog.sqf",(_traderMenu select 0), 99, true, false, "",""];
				s_player_parts set [count s_player_parts,_buy];
